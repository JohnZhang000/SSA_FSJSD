...
class EvalFhmapConfig:
    ...
    # grouped configs
    arch: schema.ArchConfig = schema.AllconvConfig  # type: ignore
    ...
    num_samples: int = -1
    topk: Tuple = (1, 5)
    weightpath: str = '/home/ubuntu204/zhangzhuang/SSA_FSJSD/snapshots_std/checkpoint.pth.tar'
...
cs.store(group="arch", name="allconv", node=schema.AllconvConfig)
...
def eval_fhmap(cfg: EvalFhmapConfig) -> None:
    ...
    arch.load_state_dict(torch.load(weightpath)['state_dict'])
    arch = arch.to(device)
    arch.eval()
    logger.info("architecture setup: done.")

    fhmap.eval_fourier_heatmap_dct(
...
@hydra.main(config_path="../config", config_name="eval_fhmap")
def eval_fhmap_channel(cfg: EvalFhmapConfig) -> None:
    """Evalutate Fourier heat map. The result is saved under outpus/eval_fhmap.
    ...
    fhmap.eval_fourier_heatmap_dct_channel(
        datamodule.input_size,
        cfg.ignore_edge_size,
        cfg.eps,
        arch,
...
@hydra.main(config_path="../config", config_name="eval_fhmap")
def eval_fhmap_channel_relative(cfg: EvalFhmapConfig) -> None:
    """Evalutate Fourier heat map. The result is saved under outpus/eval_fhmap.
    ...
    fhmap.eval_fourier_heatmap_dct_channel_relative(
        datamodule.input_size,
        cfg.ignore_edge_size,
        2.0,#cfg.eps,
        arch,
