...
from fhmap.fourier.noise_dct import AddFourierNoise_dct,AddFourierNoise_dct_channel,AddFourierNoise_dct_channel_relative
import numpy as np
...
def create_fourier_heatmap_from_error_matrix(
    ...
    return error_matrix#torch.cat([fhmap_leftside[:, :-1], fhmap_rightside], dim=1)
...
def save_fourier_heatmap(
    ...
    np.savetxt(savedir / ("fhmap_data" + suffix + ".txt"),fhmap.numpy())
    sns.heatmap(
        fhmap.numpy(),
        vmin=0.0,
        vmax=1.0,
...
def insert_fourier_noise(transforms: List, basis: torch.Tensor) -> None:
    ...
    transforms.insert(insert_index, AddFourierNoise_dct(basis))

def insert_fourier_noise_channel(transforms: List, basis: torch.Tensor) -> None:
    """Insert Fourier noise transform to given a list of transform by inplace operation.
    ...
    transforms.insert(insert_index, AddFourierNoise_dct_channel(basis))

def insert_fourier_noise_channel_relative(transforms: List, spectrum: torch.Tensor, eps: float) -> None:
    """Insert Fourier noise transform to given a list of transform by inplace operation.
    ...
    transforms.insert(insert_index, AddFourierNoise_dct_channel_relative(spectrum,eps))


def eval_fourier_heatmap_dct(
    input_size: int,
    ...
    fhmap_width: Final[int] = width - 2 * ignore_edge_size
    ...
    spectrums = fourier.get_spectrum_dct(height, width, ignore_edge_size, ignore_edge_size)
    with tqdm(
        ...
        for i, spectrum in enumerate(pbar):  # Size of basis is [height, width]
            basis = fourier.spectrum_to_basis_dct(spectrum, l2_normalize=True) * eps
...
def eval_fourier_heatmap_dct_channel(
    input_size: int,
    ignore_edge_size: int,
    eps: float,
    arch: nn.Module,
    ...
    fhmap_height: Final[int] = height - 2 * ignore_edge_size
    fhmap_width: Final[int] = width - 2 * ignore_edge_size
    fhmap_channel: Final[int] = channel
    ...
    error_matrix_dict = {
        k: torch.zeros(fhmap_channel * fhmap_height * fhmap_width).float() for k in topk
    }

    spectrums = fourier.get_spectrum_dct_channel(channel, height, width, ignore_edge_size, ignore_edge_size)
    with tqdm(
        spectrums, ncols=160, total=fhmap_channel * fhmap_height * fhmap_width
    ) as pbar:  # without total progress par might not be shown.
        for i, spectrum in enumerate(pbar):  # Size of basis is [height, width]
            basis = fourier.spectrum_to_basis_dct_channel(spectrum, l2_normalize=True) * eps

            # insert fourier noise by inpalce operation
            noised_transforms = copy.deepcopy(original_transforms)
            insert_fourier_noise_channel(noised_transforms, basis)
    ...
    fourier_heatmaps: Final[List[torch.Tensor]] = [
        create_fourier_heatmap_from_error_matrix(
            error_matrix_dict[k].view(fhmap_channel, fhmap_height, fhmap_width)
        )
        for k in topk
    ]
    ...
    if savedir:
        for k, fourier_heatmap in zip(topk, fourier_heatmaps):
            for m,fourier_heatmap_channel in enumerate(fourier_heatmap):
                save_fourier_heatmap(fourier_heatmap_channel / 100.0, savedir, f"_top{k}_channel{m}")

    return fourier_heatmaps


def eval_fourier_heatmap_dct_channel_relative(
    input_size: int,
    ignore_edge_size: int,
    eps: float,
    arch: nn.Module,
    ...
    with tqdm(
        ...
        for i, spectrum in enumerate(pbar):  # Size of basis is [height, width]
            # basis = fourier.spectrum_to_basis_dct_channel_re(spectrum, l2_normalize=True) * eps

            # insert fourier noise by inpalce operation
            noised_transforms = copy.deepcopy(original_transforms)
            insert_fourier_noise_channel_relative(noised_transforms, spectrum, eps)
