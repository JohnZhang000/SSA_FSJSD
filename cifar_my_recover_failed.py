...
def train(net, train_loader, optimizer, scheduler):
  ...
  for i, (images, targets) in enumerate(train_loader):
    ...
    logits_all,features_all=get_preds_and_features(net,torch.cat(images,0).cuda(),args.model)
    # logits_all=net(torch.cat(images,0).cuda())
    logits_clean, logits_aug1, logits_aug2 = torch.split(
          logits_all, images[0].size(0))
    features_clean, features_aug1, features_aug2 = torch.split(
          features_all, images[0].size(0))
    ...
    #     scale_aug2=scale_aug2*((~topk_aug2)*scale_epoch+topk_aug2)
      ...
      kl_div_aug1=F.kl_div(p_mixture, p_aug1, reduction='none')
      kl_div_aug2=F.kl_div(p_mixture, p_aug2, reduction='none')
      
      kl_div_aug1=scale_aug1*torch.sum(kl_div_aug1,dim=1)
      kl_div_aug2=scale_aug2*torch.sum(kl_div_aug2,dim=1)
      # print('none')
    else:
    #   # print('No scale')
      kl_div_aug1=F.kl_div(p_mixture, p_aug1, reduction='none')
      kl_div_aug2=F.kl_div(p_mixture, p_aug2, reduction='none')
      kl_div_aug1=torch.sum(kl_div_aug1,dim=1)
      kl_div_aug2=torch.sum(kl_div_aug2,dim=1)

    loss_jsd = 12 * (F.kl_div(p_mixture, p_clean, reduction='batchmean') +
                    kl_div_aug1.sum()/kl_div_aug1.shape[0] +
                    kl_div_aug2.sum()/kl_div_aug2.shape[0]) / 3.
