...
parser.add_argument('--resume', default=saved_dir, type=str, metavar='PATH',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.aug=='aprs':
        logger.info('using augmentation: aprs')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            is_best = False
    test_c(transforms.Compose([transforms.Resize(256),transforms.CenterCrop(224),transforms.ToTensor(),normalize,]),
...
def test_my(net, test_loader, criterion):
...
def test_c(test_transform, model, criterion, args):
  ...
  for corruption in CORRUPTIONS:
    # print(c)
    for s in range(1, 6):
      valdir = os.path.join(args.data+'-C', corruption, str(s))
      val_loader = torch.utils.data.DataLoader(
          ...
          pin_memory=True)
      acc1, acc5 = test_my(model, val_loader,criterion)
...
def save_checkpoint(state, is_best, filename='checkpoints/checkpoint.pth.tar', args=None):
    filename = '{}/{}_{}_checkpoint.pth.tar'.format(args.resume, args.arch, args.conf)
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, '{}/{}_{}_best.pth.tar'.format(args.resume, args.arch, args.conf))
