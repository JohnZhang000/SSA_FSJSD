...
def main():
    ...
    os.environ['CUDA_VISIBLE_DEVICES'] = 0#options['gpu']
    ...
    # if use_gpu:
    net = nn.DataParallel(net.cuda())#, device_ids=[i for i in range(len(options['gpu'].split(',')))]).cuda()
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, gamma=0.2, milestones=[30,60])#milestones=[60, 120, 160, 190]
